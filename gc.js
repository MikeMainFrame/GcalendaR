...
function calendarEntriesUI(calendarJSON) {
  ...
  var duration = new Date(gcSlots.items[0].end.dateTime);
  const diffSec = Date.now() - zero / 1000;
  const days = parseInt(diffSec / (60 * 60 * 24));
  const minutes = parseInt((diffSec - (days * 60 * 60 * 24)) / 60);
  var counting =  "T - " + days + " days " + minutes + " minutes";  
